...
def new_relation(sql='default_sql',obj1='default_sql\\list0\\position0',obj2='default_sql\\list1\\position0',password='None'):
     '''Associate any location in the SQL with another location'''
     f = open('C:\\nssql\\%s_password.nssp' % sql)
     spassword = f.read()
     f.close()
     if spassword == 'None':
          f = open('C:\\nssql\\%s_relation.nssrt' % obj1,'w',encoding='utf-8')
          f.write(obj2)
          f.close()
     else:
          if password == spassword:
